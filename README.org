...
For partitioning and formating I'm using [[https://github.com/nix-community/disko][disko]]. I hardcoded the ids of my drives to the disks.nix config (e.g. [[*Disk setup - \[\[file:hosts/zeus/disks.nix\]\[disks.nix\]\]][disk setup of the zeus host]]) for each host. Be sure to update the disks.nix with your current and/or desired disk layout and ids.
#+begin_src shell
  sudo nix \
