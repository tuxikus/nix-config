...
in
{
  ...
  config = {
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
      ...
      (defvar tuxikus/zellij-ssh-config-file "~/.ssh/config"
        "The location of the ssh config file.")
    
      (defun tuxikus/zellij-delete-all-sessions ()
        "Delete all running Zellij sessions."
        (interactive)
        (shell-command "zellij delete-all-sessions -y"))
    
      (defun tuxikus/zellij-kill-all-sessions ()
        "Kill all running Zellij sessions."
        (interactive)
        (shell-command "zellij kill-all-sessions -y"))
    
      (defun tuxikus/zellij-reset ()
        "Kill and delete all running Zellij sessions"
        (interactive)
        (tuxikus/zellij-kill-all-sessions)
      ...
      (defun tuxikus/zellij-shell-command-in-new-tab ()
        "Run shell command in a new tab."
        (interactive)
        (let ((session (tuxikus/zellij-select-session))
              (command (read-string "Command: ")))
          (shell-command (format "zellij --session %s action new-tab --name %s && zellij --session %s action write-chars '%s' && zellij action write 13" session "tab-name" session command))))
    
      (defun tuxikus/zellij-shell-command (command)
        "Run shell command COMMAND in a new tab."
        (interactive "sCommand: ")
        (let ((session (tuxikus/zellij-select-session)))
          (shell-command (format "zellij --session %s action write-chars '%s' && zellij action write 13" session command))))
    
      (defun tuxikus/zellij-ssh-in-new-tab ()
        "Open a ssh connection in a new tab."
        (interactive)
        (let ((session (tuxikus/zellij-select-session))
            (machine (tuxikus/zellij-select-machine)))
          (shell-command (format "zellij --session %s action new-tab --name %s && zellij --session %s action write-chars 'ssh %s' && zellij action write 13" session (format "ssh:%s" machine) session machine))))
    
      (defun tuxikus/zellij-kill-current-tab ()
        "Not implemented.")
    
      (defun tuxikus/zellij-kill-tab ()
        "Not implemented.")
    
      (defun tuxikus/zellij-send-region-to-current-tab ()
        "Not implemented.")
    
      (defun tuxikus/zellij-send-region-to-new-tab ()
        "Not implemented.")
    
      (defun tuxikus/zellij-select-machine ()
        "Prompt the user to select a machine from the SSH config file."
        (let ((hosts '()))
          (with-temp-buffer
            (insert-file-contents tuxikus/zellij-ssh-config-file)
            (goto-char (point-min))
            (while (re-search-forward "^Host.*" nil t)
              (push (cdr (split-string (match-string 0) " ")) hosts)))
          (completing-read "Host: " hosts)))
    
      (defun tuxikus/zellij-select-session ()
